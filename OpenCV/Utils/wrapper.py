...
def checkValidImage(src_image):
    if isinstance(src_image, str):
        try:
            src_image = imageRead(src_image)
        except:
            print("invalid image or invalid path!")
            exit()
...
def imageShow(imagename, image, flag=cv2.WINDOW_NORMAL):
...
def imageCopy(src_image):
    return np.copy(src_image)
